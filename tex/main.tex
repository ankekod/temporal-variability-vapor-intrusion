...
\usepackage{comment}
\graphicspath{{./../figures/},{./../figures/air-exchange-rate/},{./../figures/kde-iacc-pressure/}}
\usepackage{lineno}
\linenumbers
% macros
...
\begin{figure}[htb!]
  ...
  \begin{subfigure}{\textwidth}
    ...
    \includegraphics[height=0.4\textheight,keepaspectratio]{asu-nas.pdf}
  \end{subfigure}
  % Indianapolis plot
  \begin{subfigure}{\textwidth}
    \centering
...
\end{figure}
% Introduction
Out of the factors that influence the IACC in VI, pressure is one of the most dynamic ones, and the relationship between changes in indoor/outdoor pressure difference and IACC are examined in Figure \ref{fig:kde-analysis}.
...
The IACC at these sites vary significantly, therefore some means of comparing them all to each other is necessary.
Additionally, the focus in this paper is the driver for variations in IACC, and reprentation of the variations between the different sites must also be achieved, specifically variations on the scale of order of magnitudes is of the greatest interest, as these are the largest hinderance to proper site IACC characterization.
...
In Figure \ref{fig:kde-asu-nas} the North Island and ASU house datasets are plotted, with blue representing the North Island site, with red and green representing the ASU house.
The ASU house dataset is considered in two different parts, the first (red) is data belonging to the time period before the PP was discovered, the second (green) is after the PP was sealed off.
At both of these sites, TCE was the only contaminant considered.
The IACC of Chloroform and PCE the 422 part of the Indianapolis duplex are considered in Figure \ref{fig:kde-indianapolis}.\par
